...
Two checks, in order. First, confirm you are running the code you think you are:
...
After a Developer Installation this must print a path inside your cloned repo (`.../Files/hazelbean/hazelbean_dev/hazelbean/__init__.py`). If it prints something under `site-packages` instead, a non-editable copy is shadowing your clone, and none of your local changes are being used. Fix it by re-running the removal and editable install from step 3:
...
This is worth checking whenever something you know you changed appears to have no effect: the two copies are easy to confuse, because which one wins can depend on the directory you started Python from.

Second, confirm data resolution is wired up, without starting a real run:
