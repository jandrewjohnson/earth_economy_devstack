...
The `data_ref_path` is what we call a Reference Path (or Ref Path). This means it is a relative path relative to where we store our base_data (and can easily be found and downloaded by the `get_path(data_ref_path)` function.
...
5.  I will then further clean the data (to be compliant with the conventions defined here <https://justinandrewjohnson.com/earth_economy_devstack/conventions.html>, and specifically, to be a [POG](making_a_pog.qmd). I'm happy to do the cleaning, but if you want, you might want to consider following the conventions earlier in your project rather than later. 
