...
Python allows both single and double quotes for strings, but they must be *balanced*. Best practices are mixed, but we do it where the outer string always uses doublequote and then inner strings (like in the dictionary key string values below) use single quotes.
