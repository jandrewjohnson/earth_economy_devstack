---
aliases:
  - /earth_economy_devstack/project_complexity.html
---
...
**The canonical form.** When you want variants, graduate to configuration level 3: `build_task_tree(p)`, `run_project(p)`, and a `__main__` guard that builds and configures the ProjectFlow. The rule that replaces a signature full of defaults: **`run_project(p)` sets what no variant ever changes; the caller sets what a variant might.** When a constant starts varying, it moves one line up, out of `run_project` and into the caller — no signature to edit and no default to keep in sync. (One deliberate exception, amended 2026-08: the caller sets the *full* definitions-filename block as the project's interface manifest — see the conventions page.) See [`run_template_3_canonical.py`](https://github.com/jandrewjohnson/earth_economy_devstack/blob/main/examples/run_templates/run_template_3_canonical.py), its annotated twin in `examples/run_templates_annotated/` for the reasoning, and the [conventions](conventions.qmd) page for the full rules. For a project of your own that imports a devstack library as a read-only dependency, copy [`run_template_downstream_user.py`](https://github.com/jandrewjohnson/earth_economy_devstack/blob/main/examples/run_templates/run_template_downstream_user.py) into your repo as its `run_<project>.py`; for stock SEALS, copy [`run_seals.py`](https://github.com/jandrewjohnson/seals/blob/main/seals/run_seals.py). The full-scale reference implementation is `ngfs_pnas` (a private repository — ask for access).
...
def run_project(p):
    ...
    # the test wrapper points this at the pared CSV.
...
if __name__ == '__main__':
    # One call does the whole directory setup: it places the project and creates
    # it; input_template/ is read in place (input/ holds only your overrides). A
    # stable project_name with run_mode='check' means repeated runs resume in place.
    p = hb.ProjectFlow(project_name='yield', run_mode='check')
    p.parameter_definitions_filename = 'yield_parameters.csv'
