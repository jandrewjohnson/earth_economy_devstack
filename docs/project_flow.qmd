...
Project Flow is intended to flow easily from the situation where you have coded a script that grows and grows until you think "oops, I should really make this modular." Thus, it has several modalities useful to researchers ranging from simple drop-in solution to complex scripting framework. Similar to the [Project complexity](project_complexity.qmd) page, we will introduce progressively more complex usages, starting with the simples examples and working towards distributed parallelization on complex task-trees. If you would like to skip the stages and go straight to a full-functioning example, go to the [run templates](https://github.com/jandrewjohnson/earth_economy_devstack/tree/main/examples/run_templates).
...
[Project complexity](project_complexity.qmd).
